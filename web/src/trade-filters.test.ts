...
describe('filterPairsByOwners', () => {
  it('returns everything when neither selection is made', () => {
    assert.deepEqual(shape(filterPairsByOwners(ALL, ownerOf, null, null)), shape(ALL))
  })
  ...
  it('finds an owner on EITHER side from one selection', () => {
    ...
    assert.deepEqual(shape(filterPairsByOwners(ALL, ownerOf, 'Anna', null)), [
      '#AAA/#BBB',
      '#BBB/#ZZZ',
      '#AAA/#ZZZ',
      '#AAA/#QQQ',
  ...
  it('does not care which selection an owner is put in', () => {
    assert.deepEqual(
      shape(filterPairsByOwners(ALL, ownerOf, 'Anna', null)),
      shape(filterPairsByOwners(ALL, ownerOf, null, 'Anna')),
    )
  })

  it('finds a pair between two owners whichever column each is in', () => {
    const wanted = ['#AAA/#BBB', '#BBB/#ZZZ']
    assert.deepEqual(shape(filterPairsByOwners(ALL, ownerOf, 'Anna', 'Bert')), wanted)
    // Reversed selections, identical answer: the property per-column filtering lacks.
    assert.deepEqual(shape(filterPairsByOwners(ALL, ownerOf, 'Bert', 'Anna')), wanted)
  })
  ...
  it('finds trades between two bases owned by the same person', () => {
    ...
    assert.deepEqual(shape(filterPairsByOwners(ALL, ownerOf, 'Anna', 'Anna')), ['#AAA/#ZZZ'])
  })

  it('does not let one side satisfy the same owner twice', () => {
    // #AAA/#BBB has one Anna side. Asking for two must not match it.
    assert.deepEqual(filterPairsByOwners([ANNA_LEFT], ownerOf, 'Anna', 'Anna'), [])
  })

  it('matches an unowned base through the sentinel', () => {
    assert.deepEqual(shape(filterPairsByOwners(ALL, ownerOf, UNOWNED, null)), ['#AAA/#QQQ'])
  })

  it('answers empty for an owner in no pair rather than falling back to everything', () => {
    assert.deepEqual(filterPairsByOwners(ALL, ownerOf, 'Nobody', null), [])
  })

  it('preserves the order it was given', () => {
    // The table is ordered by the rules that produced it; filtering must not resequence.
    const filtered = filterPairsByOwners(ALL, ownerOf, 'Anna', null)
  ...
  it('does not mutate or alias the input', () => {
    ...
    filterPairsByOwners(input, ownerOf, null, null).length = 0
...
describe('tradeFilterSummary', () => {
  it('says nothing when nothing is filtered', () => {
    assert.equal(tradeFilterSummary(5, 5, null, null), null)
  })

  it('names one owner as involvement', () => {
    assert.equal(tradeFilterSummary(4, 5, 'Anna', null), 'Showing 4 of 5 pairs, involving Anna.')
  })

  it('names two owners as being between them', () => {
    assert.equal(
      tradeFilterSummary(2, 5, 'Anna', 'Bert'),
  ...
  it('says so plainly when a filter matches nothing', () => {
    assert.equal(tradeFilterSummary(0, 5, 'Anna', 'Carl'), 'No suggested trades between Anna and Carl.')
  })

  it('gets the singular right', () => {
    assert.equal(tradeFilterSummary(1, 1, 'Anna', null), 'Showing 1 of 1 pair, involving Anna.')
  })
})
