...
no name of their own, so on such a checkout the accessible names of the controls over and under the
frame — the badge and the two steppers — and the tile's `title` are the only things that still say
which card is which.
